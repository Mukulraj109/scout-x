...
import { waitForCloudflareIfPresent, simulateHumanMouse, applyHumanDelay } from "../services/unblocker";
...
async function gotoWithFallbackAndUnblock(page: Page, url: string) {
...
export async function convertPageToMarkdown(url: string, page: Page): Promise<string> {
  try {
    ...
    await gotoWithFallbackAndUnblock(page, url);
