...
const createDummySocket = (id: string) => ({
  ...
  nsp: {
    emit: (event: string, data?: any) => {
      logger.log('debug', `Browser ${id} dummy namespace emitted ${event}:`, data);
    },
  },
}) as any;
 ...
 * @category BrowserManagement-Controller
 */
export const createRemoteBrowserForRun = (
  userId: string,
  ...
  initializeBrowserAsync(id, userId, profile)
    .catch((error: any) => {
...
const initializeBrowserAsync = async (
  id: string,
  userId: string,
  profile?: BrowserLaunchProfile & {
    userAgent?: string;
  ...
  try {
    ...
    try {
      ...
      if (socket) {
        ...
        browserSession = new RemoteBrowser(createDummySocket(id), userId, id);
      }
      ...
      try {
        ...
        const initPromise = browserSession.initialize(userId, profile);
...
export const createRemoteBrowserForValidation = async (
  ...
  try {
    const browserSession = new RemoteBrowser(createDummySocket(id), userId, id);
