...
export const getNonUniqueSelectors = async (page: Page, coordinates: Coordinates, listSelector: string): Promise<SelectorResult> => {
  ...
  try {
    if (!listSelector) {
      ...
      const selectors = await page.evaluate(({ x, y }: { x: number, y: number }) => {
        ...
        function getNonUniqueSelector(element: HTMLElement): string {
          ...
          if (element.parentElement) {
            ...
            if (similarSiblings.length > 0) {
              // If similar siblings exist, we avoid adding :nth-child to maintain the pattern
              // This is crucial for list extraction where we want the selector to match all items
              // selector += `:nth-child(${position})`;
            }
          }
      ...
      const selectors = await page.evaluate(({ x, y, listSelector }: { x: number, y: number, listSelector: string }) => {
        const getDeepestElementFromPoint = (x: number, y: number): HTMLElement | null => {
          let elements = document.elementsFromPoint(x, y) as HTMLElement[];
          if (!elements.length) return null;
        ...
        function getNonUniqueSelector(element: HTMLElement): string {
          ...
          if (element.parentElement) {
            ...
            if (similarSiblings.length > 0) {
              // Avoid :nth-child to preserve the repeating pattern for list items
              // selector += `:nth-child(${position})`;
            }
          }
        ...
        const generalSelector = getSelectorPath(element);
        return { generalSelector };
      }, { x: coordinates.x, y: coordinates.y, listSelector });
