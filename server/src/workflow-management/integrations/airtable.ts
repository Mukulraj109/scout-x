...
async function withTokenRefresh<T>(robotId: string, apiCall: (accessToken: string) => Promise<T>): Promise<T> {
  ...
  let accessToken = robot.airtable_access_token as string;
  let refreshToken = robot.airtable_refresh_token as string;
  ...
  try {
    ...
    if (error.response?.status === 401 || 
      ...
      try {
        ...
        robot.airtable_refresh_token = tokens.refresh_token || refreshToken;
        await robot.save();
