...
class ClientSelectorGenerator {
  ...
  private normalizeClasses(classList: DOMTokenList): string {
    return Array.from(classList)
      .filter((cls) => {
        // Drop Maxun recorder-artifact classes — `__maxun_list_highlight`,
        // `__maxun_highlight`, `__maxun_selected`, etc. are injected by the
        // extension at record-time to draw selection overlays on the page and
        // MUST NOT end up in the persisted CSS selector. If they do, the saved
        // selector matches 0 items when the server later scrapes the page
        // (since those classes don't exist on the live target).
        if (cls.startsWith("__maxun_")) return false;
