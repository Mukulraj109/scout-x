...
export const BrowserContent = () => {
  ...
  const { recordingUrl } = useGlobalInfoStore();
  ...
  // Keep active tab label in sync when `recordingUrl` updates from the socket (e.g. navigation).
  useEffect(() => {
    if (!recordingUrl || recordingUrl === "https://") return;
    try {
      const parsed = new URL(recordingUrl);
      const host =
        parsed.hostname.match(/\b(?!www\.)[a-zA-Z0-9]+/g)?.join(".") || parsed.hostname;
      if (!host) return;
      setTabs((prev) => {
        if (prev.length === 0) return [host];
        const idx = Math.min(tabIndex, prev.length - 1);
        if (prev[idx] === host) return prev;
        const next = [...prev];
        next[idx] = host;
        return next;
      });
