 ...
 */
  window.scrapeList = async function ({ listSelector, fields, limit = 10 }) {
    ...
    const extractValue = (element, attribute) => {
      ...
      if (attribute === "innerText") {
        ...
        if (attribute === "href" && element.tagName !== "A") {
          // Check children first (common for titles wrapping a link)
          const childLink = element.querySelector("a");
          if (childLink) {
            const childHref = childLink.getAttribute("href");
            if (childHref) {
              try {
                return new URL(childHref, baseURL).href;
              } catch (e) {
                return childHref;
              }
            }
          ...
          // Fallback to parent (common for images inside a link)
        ...
        // Check for specific data attributes that might store the real URL
        const possibleAttrs = [attribute, "data-" + attribute, "data-src", "data-href", "data-original", "data-lazy"];
        let dataAttr = null;
    ...
    const findSimilarElements = (baseElement, similarityThreshold = 0.7) => {
      ...
      const baseTagName = baseElement.tagName;

      const allElements = [];

      // Get elements from main document
      allElements.push(...document.getElementsByTagName(baseTagName));

      // Get elements from shadow DOM
      if (baseElement.getRootNode() instanceof ShadowRoot) {
        const shadowHost = baseElement.getRootNode().host;
        allElements.push(
          ...shadowHost.getElementsByTagName(baseTagName)
        );
      }
      ...
      for (const frame of frames) {
        try {
          ...
          allElements.push(
            ...frameDoc.getElementsByTagName(baseTagName)
          );
        } catch (e) {
          console.warn(
      ...
      return allElements.filter((element) => {
        ...
        // If both have classes, use class similarity
        const elementClasses = Array.from(element.classList);
        if (baseClasses.length > 0 && elementClasses.length > 0) {
        ...
        // If no classes, fallback to tag name and parent similarity
        if (baseClasses.length === 0 && elementClasses.length === 0) {
          // Check if parents have similar tag or depth
          const baseParent = baseElement.parentElement;
          const targetParent = element.parentElement;
          if (baseParent && targetParent && baseParent.tagName === targetParent.tagName) {
            return true;
          }
        }
    ...
    if (
      ...
      if (containers.length === 1) {
        ...
        // If we found multiple, still try to find even more using the similarity pattern
        // This helps if the initial selector only matched a subset of items
        const baseContainer = containers[0];
        const similarContainers = findSimilarElements(baseContainer);

        const existingSet = new Set(containers);
        const added = similarContainers.filter(c => !existingSet.has(c));
        if (added.length > 0) {
          containers = [...containers, ...added];
          console.log(`➕ Added ${added.length} more similar containers via pattern matching`);
        }
      }
    }
 ...
 */
  ...
  /**
   * Locate the nearest overflow-scrollable ancestor of `listSelector`.
   *
   * Used by the Node-side scroll engine in `interpret.ts` + `listExtractor.ts`
   * so the cloud scraper scrolls the correct container for virtualized lists
   * (matches the Chrome extension's `findScrollRoot` helper in
   * `extractionRunner.ts`). Returns a small descriptor; the actual scroll
   * call then happens via the existing `page.evaluate` helpers.
   *
   * @param {string} listSelector CSS or XPath selector for the list items.
   * @returns {{ kind: 'window' | 'element', tag?: string, className?: string }}
   */
  window.__maxunScrollRoot = function (listSelector) {
    try {
      const isXPath = (s) =>
        typeof s === 'string' && (s.startsWith('//') || s.startsWith('/') || s.startsWith('./'));

      let anchor = null;
      if (typeof listSelector === 'string' && listSelector.length > 0) {
        if (isXPath(listSelector)) {
          try {
            const r = document.evaluate(listSelector, document, null, XPathResult.FIRST_ORDERED_NODE_TYPE, null);
            anchor = r.singleNodeValue;
          } catch (_) {
            anchor = null;
          }
        } else {
