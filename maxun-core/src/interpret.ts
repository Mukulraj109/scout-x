 ...
 */
/**
 * Optional hooks the host (e.g. the Maxun server) can inject so Engine 1
 * (recorded robots) gets the same overlay / dialog / captcha treatment the
 * Chrome extension's scraper applies locally.
 *
 * All hooks are optional and must never throw in the happy path — they're
 * invoked between pagination iterations and before extraction. To signal a
 * captcha, throw a `CaptchaEncounteredError` (or any error whose `name` is
 * `'CaptchaEncounteredError'`) from `assertNoCaptcha` and the interpreter
 * will propagate it to the caller without swallowing.
 */
export interface PaginationHooks {
  /** Run a cookie-banner / modal dismissal pass. */
  dismissOverlays?: (page: Page) => Promise<void> | void;
  /** Throw if a CAPTCHA / challenge is visible; no-op otherwise. */
  assertNoCaptcha?: (page: Page) => Promise<void> | void;
  /** Install page-level `dialog` handler (accept/dismiss alert/confirm/prompt). */
  installDialogHandler?: (page: Page) => (() => void) | void;
}
 ...
 */
    ...
    const wawActions: Record<CustomFunctions, (...args: any[]) => void> = {
      ...
      scrapeList: async (config: { listSelector: string, fields: any, limit?: number, pagination: any }, actionName: string = "") => {
        ...
        try {
          ...
          // A CAPTCHA encountered during pagination must propagate up to the
          // worker so the run can be paused and a `captcha:required` event
          // can be emitted. Everything else is isolated so one bad
          // scrapeList doesn't sink the whole workflow.
          if (error && (error.name === 'CaptchaEncounteredError' || error?.isCaptchaEncountered === true)) {
            throw error;
          }
  ...
  private async handlePagination(page: Page, config: { 
    ...
    const hooks: PaginationHooks = this.options.paginationHooks || {};
    ...
    /**
     * Stable row fingerprint. Mirrors the Chrome extension's `fingerprintRow`
     * in `extractionRunner.ts` so the same row extracted locally and in the
     * cloud collapses to the same identity.
     */
    const fingerprintRow = (row: Record<string, any>): string =>
      JSON.stringify(
    ...
    /** Run overlay-dismiss + captcha checks if the host provided hooks. */
    const runGuards = async () => {
      if (hooks.dismissOverlays) {
        try { await hooks.dismissOverlays(page); } catch { /* ignore */ }
      }
      if (hooks.assertNoCaptcha) {
        // Let captcha errors propagate — runLoop / public run() will translate.
        await hooks.assertNoCaptcha(page);
      }
    };

    /** Count items matching the list selector (CSS or XPath). */
    const countListItems = async (): Promise<number> => {
      try {
    ...
    /** Find the list's scroll container and return its scrollHeight. */
        ...
        return await page.evaluate((sel: string) => {
          ...
          try {
            if (sel.startsWith('//') || sel.startsWith('/') || sel.startsWith('./')) {
              ...
              anchor = r.singleNodeValue as Element | null;
            } else {
    ...
    /** Scroll the list's root (inner or window) one step. */
    const stepScroll = async (direction: 'down' | 'up' = 'down', mode: 'viewport' | 'toEnd' = 'toEnd'): Promise<boolean> => {
      try {
        return await page.evaluate(({ sel, dir, m }: { sel: string; dir: 'down' | 'up'; m: 'viewport' | 'toEnd' }) => {
          const findRoot = (el: Element | null): HTMLElement => {
            const pageRoot = (document.scrollingElement || document.documentElement) as HTMLElement;
            if (!el) return pageRoot;
            let cur: HTMLElement | null = el as HTMLElement;
          ...
          const target = findRoot(anchor);
          const isPageRoot = target === document.scrollingElement || target === document.documentElement || target === document.body;
          ...
          if (isPageRoot) {
            ...
            const nextY = m === 'toEnd'
              ? (dir === 'down' ? maxY : 0)
              : (dir === 'down'
                ? Math.min(before + Math.max(200, vp * 0.85), maxY)
                : Math.max(before - Math.max(200, vp * 0.85), 0));
            window.scrollTo({ top: nextY, left: 0, behavior: 'instant' as ScrollBehavior });
            return Math.abs(window.scrollY - before) > 1;
          }

          const before = target.scrollTop;
          const vp = target.clientHeight;
          const maxY = Math.max(0, target.scrollHeight - vp);
          const nextY = m === 'toEnd'
            ? (dir === 'down' ? maxY : 0)
            : (dir === 'down'
              ? Math.min(before + Math.max(200, vp * 0.85), maxY)
        ...
        }, { sel: config.listSelector, dir: direction, m: mode });
      } catch {
        return false;
      }
    };

    /** Poll for more items (count or scrollHeight growth). */
    const waitForMoreItems = async (
      beforeCount: number,
      beforeSH: number,
      timeoutMs = 6000
      ...
      while (Date.now() < deadline) {
        ...
        if (count > beforeCount) return { changed: true, newCount: count, reason: 'items' };
        const sh = await getScrollHeight();
        if (sh > beforeSH + 50) return { changed: true, newCount: count, reason: 'height' };
        await page.waitForTimeout(150).catch(() => undefined);
      }
      return { changed: false, newCount: beforeCount, reason: 'timeout' };
    };

    /** Wait up to timeoutMs for any visible spinner/skeleton to disappear. */
    const waitForLoadingToFinish = async (timeoutMs = 6000, appearWindowMs = 300): Promise<boolean> => {
      try {
        return await page.evaluate(async ({ sel, total, appear }: { sel: string; total: number; appear: number }) => {
          ...
          const detect = (): Element | null => {
            const roots: Element[] = [];
            if (root) { roots.push(root); if (root.parentElement) roots.push(root.parentElement); }
            ...
            for (const r of roots) {
              ...
              try {
                cands = [
                  ...Array.from(r.querySelectorAll('[aria-busy="true"]')),
                  ...Array.from(r.querySelectorAll('[role="progressbar"], [role="status"]')),
                ];
              } catch { /* ignore */ }
              try {
                const all = Array.from(r.querySelectorAll<HTMLElement>('*'));
                for (const e of all) {
                  if (typeof e.className === 'string' && classMatch.test(e.className)) cands.push(e);
                }
              } catch { /* ignore */ }
          ...
          return false;
        }, { sel: config.listSelector, total: timeoutMs, appear: appearWindowMs });
      } catch {
        return false;
      }
    };

    /** Snapshot signals used to detect page changes after a click. */
    const snapshotPage = async (): Promise<{ url: string; firstItem: string; itemCount: number; htmlLen: number }> => {
      try {
        return await page.evaluate((sel: string) => {
          let first = '';
          let count = 0;
          try {
            if (sel.startsWith('//') || sel.startsWith('/') || sel.startsWith('./')) {
              ...
              if (count > 0) first = ((r.snapshotItem(0) as Element)?.textContent || '').trim().slice(0, 100);
            } else {
              const nodes = document.querySelectorAll(sel);
              count = nodes.length;
              if (count > 0) first = (nodes[0].textContent || '').trim().slice(0, 100);
            }
          } catch { /* ignore */ }
          return {
        ...
        return { url: '', firstItem: '', itemCount: 0, htmlLen: 0 };
      }
    };

    /** Poll for any page-change signal (URL, itemCount, firstItem, HTML size). */
    const waitForPageChange = async (
      before: { url: string; firstItem: string; itemCount: number; htmlLen: number },
      timeoutMs = 12000
    ): Promise<boolean> => {
      const deadline = Date.now() + timeoutMs;
      await page.waitForTimeout(100).catch(() => undefined);
      while (Date.now() < deadline) {
        const now = await snapshotPage();
        if (now.url && now.url !== before.url) return true;
        if (before.itemCount > 0 && now.itemCount !== before.itemCount) return true;
        if (now.firstItem && now.firstItem !== before.firstItem) return true;
        if (before.htmlLen > 0) {
    ...
    const scrapeCurrentPage = async () => {
        ...
        // Dismiss overlays + check CAPTCHA before every extraction pass so we
        // don't scrape a cookie banner or a challenge page.
        await runGuards();
        ...
        const newResults = results.filter(item => {
            // Stable whitespace/case-insensitive fingerprint — matches the
            // extension so dedup is consistent end-to-end.
            const uniqueKey = fingerprintRow(item);
    ...
    try {
      while (true) {
        ...
        switch (config.pagination.type) {
          case 'scrollDown': {
            const previousResultCount = allResults.length;
            ...
            // Parity with the extension's auto-scroll:
            //   1) find the list's actual scroll container (window or inner),
            //   2) step-scroll to the end of currently-loaded content,
            //   3) wait for any spinner/skeleton to clear,
            //   4) poll for item-count or scrollHeight growth.
            const beforeCount = await countListItems();
            const beforeSH = await getScrollHeight();

            const moved = await stepScroll('down', 'toEnd');

            const spinnerBudget = Math.max((config.pagination.pageDelayMs || 1500) * 3, 8000);
            await waitForLoadingToFinish(spinnerBudget, 300);

            const growth = await waitForMoreItems(
              beforeCount,
              beforeSH,
              Math.max(1500, config.pagination.pageDelayMs || 2000)
            );
            ...
            const madeProgress = growth.changed || currentResultCount > previousResultCount || moved;

            if (!madeProgress) {
              unchangedResultCounter++;
              if (unchangedResultCounter >= MAX_UNCHANGED_RESULTS) {
                debugLog(`scrollDown: no growth for ${MAX_UNCHANGED_RESULTS} iterations — stopping`);
                return allResults;
              }
            } else {
          ...
          case 'scrollUp': {
            ...
            const moved = await stepScroll('up', 'toEnd');
            ...
            if (!madeProgress) {
              unchangedResultCounter++;
              if (unchangedResultCounter >= MAX_UNCHANGED_RESULTS) {
                debugLog(`scrollUp: no growth for ${MAX_UNCHANGED_RESULTS} iterations — stopping`);
                return allResults;
              }
            } else {
              unchangedResultCounter = 0;
            ...
            const currentTopHeight = await page.evaluate(() => document.documentElement.scrollTop).catch(() => 0);
            if (currentTopHeight === 0 && !growth.changed) {
              return allResults;
            }
          ...
          case 'pageNumber': {
            // URL-parameter pagination. Mirrors listExtractor.paginateByPageNumber
            // on the Engine 2 side so extension-recorded robots that use a
            // `?page=N` style URL work end-to-end.
            await scrapeCurrentPage();
            ...
            const pageParam = (config.pagination.pageParam as string) || 'page';
            const startPage = typeof config.pagination.startPage === 'number'
              ? config.pagination.startPage
              : 1;
            const maxPages = typeof config.pagination.maxPages === 'number' && config.pagination.maxPages > 0
            ...
            while (currentPageIndex < maxPages) {
              ...
              let target: URL;
              try {
                target = new URL(page.url() || startUrl);
              } catch {
                debugLog('pageNumber: invalid URL, stopping');
                return allResults;
              }
              const nextPageNumber = startPage + currentPageIndex;
              target.searchParams.set(pageParam, String(nextPageNumber));
              const nextUrl = target.toString();

              if (nextUrl === page.url() || visitedUrls.has(nextUrl)) {
                debugLog(`pageNumber: target URL already visited or unchanged (${nextUrl})`);
                return allResults;
              }

              try {
                await page.goto(nextUrl, { waitUntil: 'networkidle', timeout: 100_000 });
              } catch {
                try {
                  await page.goto(nextUrl, { waitUntil: 'domcontentloaded', timeout: 100_000 });
                } catch (navError: any) {
                  debugLog(`pageNumber: goto failed for ${nextUrl}: ${navError?.message || navError}`);
                  return allResults;
                }
              }
              visitedUrls.add(nextUrl);

              await page.waitForTimeout(Math.max(500, config.pagination.pageDelayMs || 1200));
              ...
              if (allResults.length === previousResultCount) {
                unchangedResultCounter++;
                if (unchangedResultCounter >= MAX_UNCHANGED_RESULTS) {
                  debugLog('pageNumber: no new rows across recent pages — stopping');
                  return allResults;
                }
              } else {
                unchangedResultCounter = 0;
          ...
          case 'clickNext': {
            ...
            // Snapshot BEFORE the click so waitForPageChange can tell whether
            // the SPA actually navigated / re-rendered the list area.
            const preClickSnapshot = await snapshotPage();
            ...
            while (retryCount < MAX_RETRIES && !paginationSuccess) {
              try {
                ...
                // Use the richer waitForPageChange poll (URL, itemCount, firstItem,
                // HTML size) instead of only comparing snapshots once. Matches
                // the Chrome extension's `waitForPageChange` behaviour.
                const changed = await waitForPageChange(preClickSnapshot, 12000);
                if (changed) {
                  const newUrl = page.url();
                  ...
                  // Fallback: compare the coarser signatures in case the DOM
                  // change was too subtle to be caught by the poll.
                  const newUrl = page.url();
          ...
          case 'clickLoadMore': {
            ...
            const loadMoreWait = Math.max(800, config.pagination.pageDelayMs || 2000);
            ...
            while (true) {
              ...
              // Snapshot before clicking so we can verify the click actually
              // loaded more items.
              const preClickSnapshot = await snapshotPage();
              const beforeSH = await getScrollHeight();
              ...
              // Poll for page change (URL, itemCount, firstItem, HTML size)
              // so we don't rely on a fixed 4-second wait.
              await waitForPageChange(preClickSnapshot, loadMoreWait);

              // Give the site a chance to finish any spinner-gated load.
              const spinnerBudget = Math.max(loadMoreWait * 2, 6000);
              await waitForLoadingToFinish(spinnerBudget, 300);

              // Scroll the list container to trigger downstream lazy-loads.
              await stepScroll('down', 'toEnd');
              await page.waitForTimeout(300);

              const currentHeight = await getScrollHeight();
              const heightChanged = currentHeight > 0 && currentHeight !== beforeSH && currentHeight !== previousHeight;
              previousHeight = currentHeight;
        ...
        // CAPTCHA surfaced from the `runGuards` hook must propagate so the
        // worker can pause the run and emit `captcha:required`. Any other
        // fatal error is swallowed (log + return partial results) to avoid
        // breaking a recorded robot on transient page issues.
        if (error && (error.name === 'CaptchaEncounteredError' || error?.isCaptchaEncountered === true)) {
          throw error;
        }
  ...
  public async run(page: Page, params?: ParamType): Promise<void> {
    ...
    // Install pagination hooks (overlay dismissal, dialog handling, captcha
    // detection) once per run. Caller is expected to pass `paginationHooks`
    // in `options` — see PaginationHooks JSDoc at the top of this file.
    let disposeDialogHandler: (() => void) | null = null;
    const hooks: PaginationHooks = this.options.paginationHooks || {};
    try {
      if (hooks.installDialogHandler) {
        const disposer = hooks.installDialogHandler(page);
        if (typeof disposer === 'function') {
          disposeDialogHandler = disposer;
        }
      }
      if (hooks.dismissOverlays) {
        try { await hooks.dismissOverlays(page); } catch { /* never let overlay dismissal abort a run */ }
      }
    } catch (hookError: any) {
      this.log(`Pagination hooks setup failed: ${hookError?.message || hookError}`, Level.WARN);
    }
    ...
    try {
      ...
      if (disposeDialogHandler) {
        try { disposeDialogHandler(); } catch { /* ignore */ }
      }
    }
